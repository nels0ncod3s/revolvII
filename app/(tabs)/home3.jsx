...
// Format a date string nicely: "Sat 21 Mar 26"
...
function getTimeData() {
  ...
  let period, greeting;
  if      (h >= 5  && h < 12) { period = "morning";   greeting = "Good Morning";   }
  else if (h >= 12 && h < 16) { period = "afternoon";  greeting = "Good Afternoon"; }
  else if (h >= 16 && h < 20) { period = "evening";    greeting = "Good Evening";   }
  else if (h >= 20 && h < 24) { period = "night";      greeting = "Relax Tonight";  }
  else                         { period = "late";       greeting = "You're Up Late"; }

  const message = Math.random() < 0.3
    ? RANDOM_MESSAGES[Math.floor(Math.random() * RANDOM_MESSAGES.length)]
    : greeting;

  const gradientColors = ["#f7f6f3", "#f3f2ef", "#f7f6f3"];
  return { period, message, gradientColors };
}

// ─── Spend card wrapper ───────────────────────────────────────────────────────
function SpendCard({ children, onPress }) {
  ...
  return (
    <Animated.View style={[sc.wrap, { transform: [{ scale: pressAnim }] }]}>
      <LinearGradient colors={["#ffffff", "#faf9f7"]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={StyleSheet.absoluteFill} />
      <LinearGradient colors={["rgba(124,92,255,0.06)", "rgba(124,92,255,0)"]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={sc.tint} />
...
const sc = StyleSheet.create({
  wrap: {
    borderRadius: 28, overflow: "hidden", marginBottom: 14,
    borderWidth: 1, borderColor: "rgba(0,0,0,0.06)",
    shadowColor: "#7c5cff", shadowOpacity: 0.08, shadowRadius: 20,
    shadowOffset: { width: 0, height: 6 }, elevation: 4,
  },
  tint:    { position: "absolute", top: 0, left: 0, right: 0, height: 60 },
  content: { padding: 24 },
});

// ─── Animated new-card entrance ───────────────────────────────────────────────
// isNew=true  → animates in from below (freshly added card)
// isNew=false → renders instantly (existing cards on screen load)
...
// ─── Quick-info bottom sheet (tap card) ───────────────────────────────────────
// Shows the Netflix-style detail sheet from the screenshot
function QuickInfoSheet({ item, visible, onClose, onEdit, currency, formatPrice }) {
  ...
  const subscribed = daysSince(item.startDate);
  // total spent = amount × number of billing periods since start
  let periodsElapsed = 1;
  if (item.startDate) {
    const d = daysSince(item.startDate);
    if (item.cycle === "Weekly")  periodsElapsed = Math.max(1, Math.round(d / 7));
    else if (item.cycle === "Yearly") periodsElapsed = Math.max(1, Math.round(d / 365));
    else periodsElapsed = Math.max(1, Math.round(d / 30));
  }
  const totalSpent = (item.amount || 0) * periodsElapsed;

  // Currency label
  const currCode   = item.amountCurrency || "NGN";
  ...
  return (
    <Modal visible={visible} transparent animationType="none" onRequestClose={onClose}>
      ...
      <Animated.View style={[qi.sheet, { transform: [{ translateY }] }]}>
        {/* Handle */}
        <View style={qi.handleWrap}>
          <View style={qi.handle} />
        </View>

        {/* Dark header area */}
        <View style={qi.headerBg}>
        {/* Header: icon + name + price + edit button */}
        <View style={qi.header}>
          <View style={qi.iconWrap}>
            <Text style={qi.iconEmoji}>{safeEmoji(item.icon)}</Text>
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={qi.section}>
          <InfoRow label="Currency"     value={`${currCode} · ${currSymbol}`} />
          <InfoRow label="Payment cycle" value={item.cycle} />
          <InfoRow label="Next payment"  value={fmtDate(item.billingDate)} last />
        </View>

        {/* Info rows — section 2 */}
        <View style={[qi.section, { marginTop: 10 }]}>
          <InfoRow label="Total spent"    value={`${currSymbol}${formatPrice(totalSpent)}`} />
          <InfoRow label="Subscribed for" value={`${subscribed} day${subscribed !== 1 ? "s" : ""}`} />
          <InfoRow label="Last payment"   value={fmtDate(item.startDate)} />
          <InfoRow label="Category"       value={item.category || "—"} last />
        </View>
...
const SubscriptionCard = React.memo(({
  item, onTap, onLongPress, onRequestDelete, currency, formatPrice,
}) => {
      ...
      <Animated.View style={{ transform: [{ scale }] }}>
        ...
        >
          ...
          <View style={styles.cardRight}>
            <Text style={styles.cardPrice}>{currency?.symbol}{formatPrice(item.amount)}</Text>
...
// ─── Cancelled card ───────────────────────────────────────────────────────────
const CancelledCard = ({ item, onReactivate, currency, formatPrice }) => {
  const scale = useRef(new Animated.Value(1)).current;
  ...
  return (
    <Animated.View style={{ transform: [{ scale }] }}>
      ...
      >
        ...
        <Text style={[styles.cardPrice, { color: "#94a3b8" }]}>
          {currency?.symbol}{formatPrice(item.amount)}
        </Text>
      </Pressable>
    </Animated.View>
  );
...
export default function Home() {
  ...
  const [quickItem, setQuickItem]       = useState(null);  // tap → quick info
  const [showQuick, setShowQuick]       = useState(false);
  const [showAvatar, setShowAvatar]     = useState(false);
  const [showDropdown, setShowDropdown] = useState(false);
  const [deletingId, setDeletingId]     = useState(null);
  const dropdownAnim  = useRef(new Animated.Value(0)).current;
  // Track IDs seen on initial mount — only IDs added after that are "new"
  const seenIds = useRef(null);
  if (seenIds.current === null) {
    seenIds.current = new Set(subscriptions.map((s) => s.id));
  }
  ...
  const toggleDropdown = () => {
    if (showDropdown) {
      Animated.timing(dropdownAnim, { toValue: 0, duration: 220, useNativeDriver: true })
  ...
  const handleAnimDone = useCallback((id) => {
    setDeletingId(null);
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      {/* Background */}
      <LinearGradient colors={timeData.gradientColors} style={{ flex: 1 }}>
        <SafeAreaView style={styles.safe}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>

            {/* ── Header ── */}
            <Animated.View style={[styles.header, { opacity: headerFade, transform: [{ translateY: headerSlide }] }]}>
              <TouchableOpacity style={styles.avatarBtn} onPress={() => setShowAvatar(true)} activeOpacity={0.8}>
                <Text style={{ fontSize: 24 }}>{profile?.avatar || "🧑🏽"}</Text>
              </TouchableOpacity>
              <View style={{ flex: 1 }}>
                <Text style={[styles.greetingText, { color: subColor }]}>{timeData.message},</Text>
                <Text style={[styles.nameText, { color: textColor }]}>{profile?.name || "User"}</Text>
              </View>
              {/* Full circle add button with 1.5px border */}
              <TouchableOpacity style={styles.addBtn} onPress={() => addRef.current?.expand()}>
                <Ionicons name="add" size={26} color={ACCENT} />
              </TouchableOpacity>
            </Animated.View>

            {/* ── Spend card ── */}
            <SpendCard onPress={toggleDropdown}>
              <View style={styles.spendTop}>
                <View style={styles.spendLblRow}>
                  <View style={styles.spendDot} />
                  <Text style={[styles.spendLbl, { color: subColor }]}>TOTAL MONTHLY SPEND</Text>
                </View>
                <Animated.View style={{
                  transform: [{ rotate: dropdownAnim.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "180deg"] }) }]
                }}>
                  <Ionicons name="chevron-down" size={15} color={subColor} />
                </Animated.View>
              </View>
              ...
              <View style={styles.spendAmountRow}>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: "row", alignItems: "flex-start" }}>
                    ...
                    <Text style={[styles.spendAmount, { color: textColor }]}>{formatPrice(totalSpend)}</Text>
                  </View>
                  <Text style={[styles.spendSub, { color: subColor }]}>
                ...
                <Animated.View style={{ opacity: dropdownAnim }}>
                  ...
                  <View style={styles.breakdown}>
                    <View style={styles.bItem}>
                      <Text style={[styles.bLbl, { color: subColor }]}>MONTHLY</Text>
                      <Text style={[styles.bVal, { color: textColor }]}>{currency?.symbol}{formatPrice(totalSpend)}</Text>
                    </View>
                    <View style={[styles.bSep, { backgroundColor: "rgba(0,0,0,0.07)" }]} />
                    <View style={styles.bItem}>
                      <Text style={[styles.bLbl, { color: subColor }]}>YEARLY</Text>
                      <Text style={[styles.bVal, { color: textColor }]}>{currency?.symbol}{formatPrice(yearlyTotal)}</Text>
                    </View>
                    <View style={[styles.bSep, { backgroundColor: "rgba(0,0,0,0.07)" }]} />
                    <View style={styles.bItem}>
                      <Text style={[styles.bLbl, { color: "#10b981" }]}>SAVED</Text>
                      <Text style={[styles.bVal, { color: "#10b981" }]}>{currency?.symbol}{formatPrice(savedTotal)}</Text>
                    </View>
                  </View>
                </Animated.View>
              )}
            ...
            <View style={styles.pills}>
              <View style={[styles.pill, { backgroundColor: cardBg, borderColor: cardBorder }]}>
                ...
                <Text style={[styles.pillTxt, { color: textColor }]}>{activeSubs.length} Active</Text>
              </View>
              {savedTotal > 0 && (
                <View style={[styles.pill, { backgroundColor: cardBg, borderColor: cardBorder }]}>
                  <Ionicons name="leaf" size={12} color="#10b981" />
                  <Text style={[styles.pillTxt, { color: textColor }]}>
                    Saving {currency?.symbol}{formatPrice(savedTotal)}/mo
                  </Text>
                </View>
              )}
            </View>
            ...
            {activeSubs.length > 0 ? (
              <>
                <Text style={[styles.sectionTitle, { color: textColor }]}>Subscriptions</Text>
                {activeSubs.map((item, index) => (
                  <DeletableCard
                    key={item.id}
                    deleting={deletingId === item.id}
                    onAnimDone={() => handleAnimDone(item.id)}
                  >
              ...
              <View style={styles.empty}>
                <View style={[styles.emptyIcon, { backgroundColor: cardBg, borderColor: cardBorder }]}>
                  <Ionicons name="wallet-outline" size={36} color={ACCENT} />
                </View>
                <Text style={[styles.emptyTitle, { color: textColor }]}>No subscriptions yet</Text>
                <Text style={[styles.emptySub, { color: subColor }]}>Tap + to track your first one</Text>
            ...
            {cancelledSubs.length > 0 && (
              <>
                <Text style={[styles.sectionTitle, {
                  color: subColor, fontSize: 12, fontWeight: "700",
...
const sw = StyleSheet.create({
  ...
  deleteBtn: {
    width: 76, height: "100%", borderRadius: 18,
...
const styles = StyleSheet.create({
  ...
  // Full circle, border only, no fill
  addBtn: {
  ...
  spendAmountRow: { flexDirection: "row", alignItems: "flex-end", marginTop: 4, gap: 12 },
  ...
  cardIcon:    { width: 48, height: 48, borderRadius: 16, backgroundColor: "#f4f3ff", alignItems: "center", justifyContent: "center", marginRight: 14 },
  ...
  cardCycle:   { fontSize: 10, fontWeight: "600", marginTop: 2, color: "#64748b" },
