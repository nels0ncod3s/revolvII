...
const goHome = (router) => router.replace("/(tabs)/home");
...
export default function NotificationsSetup() {
  ...
  const handleEnable = async () => {
    try {
      ...
      setTimeout(() => goHome(router), 1000);
    }
  };

  const handleSkip = () => goHome(router);
    ...
    <View style={styles.container}>
      <View style={styles.top}>
        ...
        <Text style={styles.subtitle}>
          Revolv can remind you before a subscription renews — so you're always
          in control.
        </Text>
        <View style={styles.exampleWrap}>
          {[
            "Netflix renews in 3 days",
            "Spotify billed tomorrow",
            "iCloud renewed today",
          ].map((ex) => (
